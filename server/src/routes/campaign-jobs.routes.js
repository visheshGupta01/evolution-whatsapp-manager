...
function validateCampaign(body) {
  ...
  if (!['text', 'media', 'media-text', 'buttons', 'list', 'media-buttons', 'media-list'].includes(type)) {
    throw Object.assign(new Error('Unsupported campaign type.'), { status: 400 });
  }
  ...
  if (['buttons', 'media-buttons'].includes(type) && (!String(payload.title || '').trim() || !Array.isArray(payload.buttons) || !payload.buttons.length || payload.buttons.length > MAX_BUTTONS)) {
    throw Object.assign(new Error('Button campaign requires a title and 1–3 buttons.'), { status: 400 });
  }
  if (['list', 'media-list'].includes(type)) {
    const sections = Array.isArray(payload.sections) ? payload.sections : [];
    const rows = sections.reduce((n, s) => n + (Array.isArray(s.rows) ? s.rows.length : 0), 0);
    if (!String(payload.title || '').trim() || !String(payload.buttonText || '').trim() || !sections.length || rows < 1 || rows > MAX_LIST_ROWS) {
  ...
  try {
      ...
      pool.query(`
        ...
        delivery_daily AS (
          SELECT DATE(m.status_updated_at) AS day,
            COUNT(DISTINCT (m.campaign_id, m.recipient_index)) FILTER (WHERE m.status IN ('DELIVERY_ACK','READ','PLAYED'))::int AS delivered,
            COUNT(DISTINCT (m.campaign_id, m.recipient_index)) FILTER (WHERE m.status IN ('READ','PLAYED'))::int AS read
            ...
            (SELECT COUNT(*) FROM (
              SELECT recipient_index, MAX(
                CASE status
                  ...
                  WHEN 'SERVER_ACK' THEN 3 WHEN 'PENDING' THEN 2 WHEN 'ERROR' THEN 1 ELSE 0
                END
              ) AS rank
              FROM campaign_messages WHERE campaign_id = c.id GROUP BY recipient_index
    ...
    res.json({
      ...
      campaigns: campaignRows.rows.map((r) => ({
        ...
        total: Number(r.total), sent: Number(r.sent), failed: Number(r.failed),
        messageCount: Number(r.message_count), delivered: Number(r.delivered), read: Number(r.read),
...
campaignJobsRouter.post('/:id/pause', async (req, res, next) => {
  try {
    ...
    return res.json(await updateCampaign(campaign.id, { status: 'paused' }));
  } catch (error) { next(error); }
});
...
campaignJobsRouter.post('/:id/resume', async (req, res, next) => {
  try {
    ...
    return res.json(await updateCampaign(campaign.id, { status: 'queued' }));
  } catch (error) { next(error); }
});
...
campaignJobsRouter.post('/:id/cancel', async (req, res, next) => {
  try {
    ...
    return res.json(await updateCampaign(campaign.id, { status: 'cancelled' }));
  } catch (error) { next(error); }
});
