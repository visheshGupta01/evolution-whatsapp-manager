...
function parseRecipientSheet(data: ArrayBuffer): RecipientRow[] {
  ...
  return rows.map((row, index) => {
    ...
    return { row: index + 2, phone, name: getValue(row, 'name') || undefined, company: getValue(row, 'company') || undefined, custom1: getValue(row, 'custom1') || undefined, custom2: getValue(row, 'custom2') || undefined, valid: !error, error: error || undefined };
  });
}

function downloadTemplate() {
  const rows = [{ phone: '919876543210', name: 'Rahul', company: 'Acme', custom1: 'Premium', custom2: 'Delhi' }, { phone: '919812345678', name: 'Priya', company: 'Beta Ltd', custom1: 'New', custom2: 'Mumbai' }];
...
export function SendMessagePage() {
  ...
  const handleRecipientFile = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setIsParsing(true); setResult(null);
    try { if (!/\.(xlsx|xls|csv)$/i.test(file.name)) throw new Error('Please upload an XLSX, XLS or CSV file.'); const rows = parseRecipientSheet(await file.arrayBuffer()); if (rows.length > MAX_RECIPIENTS) throw new Error(`This phase supports up to ${MAX_RECIPIENTS} spreadsheet rows per campaign.`); setRecipients(rows); setFileName(file.name); toast.success(`Imported ${rows.length} recipient rows`); }
    catch (error) { setRecipients([]); setFileName(''); toast.error(error instanceof Error ? error.message : 'Could not read the spreadsheet.'); }
    finally { setIsParsing(false); }
  };

  const handleMediaFile = async (event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]; event.target.value = ''; if (!file) return;
    setIsMediaLoading(true); setResult(null);
    try { if (file.size > MAX_MEDIA_BYTES) throw new Error('Media must be 8 MB or smaller in Phase 3.'); const mediatype = mediaTypeFor(file); if (!mediatype) throw new Error('Supported media: images, videos, PDF and common office/text documents.'); const base64 = await readFileBase64(file); const next: MediaDraft = { base64, mediatype, mimetype: file.type || 'application/octet-stream', fileName: file.name, size: file.size }; if (file.type.startsWith('image/') || file.type.startsWith('video/')) next.previewUrl = URL.createObjectURL(file); setMedia(next); toast.success(`${file.name} attached`); }
    catch (error) { setMedia(null); toast.error(error instanceof Error ? error.message : 'Could not load media.'); }
    finally { setIsMediaLoading(false); }
  };

  const clearMedia = () => { if (media?.previewUrl) URL.revokeObjectURL(media.previewUrl); setMedia(null); setResult(null); };
  const clearRecipients = () => { setRecipients([]); setFileName(''); setResult(null); };
  ...
  const sendCampaign = async () => {
    ...
    try {
      ...
      const response = isMediaMode ? await campaignsApi.sendMedia(selectedInstance, media!, captionEnabled ? draft.trim() : '', cleanRecipients, 1500) : await campaignsApi.sendText(selectedInstance, draft.trim(), cleanRecipients, 1500);
      setResult(response); if (response.failed) toast.warning(`${response.sent} sent, ${response.failed} failed`); else toast.success(`${response.sent} messages sent`);
