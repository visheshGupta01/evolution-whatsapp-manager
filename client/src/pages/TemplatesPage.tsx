...
function TemplateIcon({ type }: { type: TemplateType }) {
  ...
  if (type === 'media-text') return <FileText size={18} />;
    ...
    {!templates.length ? <section className="templateEmpty">
      ...
      {templates.map((template) => <article className="templateCard" key={template.id}>
        <div className="templateCardTop">
          ...
          <div className="templateCardMeta"><span>{templateTypeLabel(template.type)}</span><strong>{template.name}</strong></div>
...
function TemplateEditor({ template, close, saved }: { template: MessageTemplate | null; close: () => void; saved: () => void }) {
  ...
  const save = () => {
    ...
    if (type === 'buttons' && (!title.trim() || !buttons.length || buttons.some((button) => !button.id.trim() || !button.displayText.trim()))) return toast.error('Complete the button template.');
    if (type === 'list' && (!title.trim() || !buttonText.trim() || !sections.length || sections.some((section) => !section.title.trim() || !section.rows.length || section.rows.some((row) => !row.rowId.trim() || !row.title.trim())))) return toast.error('Complete the list template.');
    ...
    if (template) updateTemplate(template.id, data);
  ...
  return <div className="templateModalBackdrop" onMouseDown={(event) => event.target === event.currentTarget && close()}>
    <section className="templateModal">
      ...
      <div className="templateModalBody">
        ...
        {type === 'buttons' && <div className="templateEditorItems"><div className="templateEditorLabel">Reply buttons</div>{buttons.map((button, index) => <div className="templateItemRow" key={index}><input value={button.id} onChange={(event) => setButtons((items) => items.map((item, i) => i === index ? { ...item, id: event.target.value } : item))} placeholder="button_id" /><input value={button.displayText} onChange={(event) => setButtons((items) => items.map((item, i) => i === index ? { ...item, displayText: event.target.value } : item))} placeholder="Button text" /><button type="button" disabled={buttons.length <= 1} onClick={() => setButtons((items) => items.filter((_, i) => i !== index))}><Trash2 size={13} /></button></div>)}{buttons.length < 3 && <button type="button" className="secondary" onClick={() => setButtons((items) => [...items, { type: 'reply', id: `option_${items.length + 1}`, displayText: `Option ${items.length + 1}` }])}><Plus size={13} /> Add button</button>}</div>}
        {type === 'list' && <div className="templateEditorItems"><label>Menu button text<input value={buttonText} onChange={(event) => setButtonText(event.target.value)} /></label>{sections.map((section, sectionIndex) => <div className="templateListSection" key={sectionIndex}><div className="templateItemRow"><input value={section.title} onChange={(event) => setSections((items) => items.map((item, i) => i === sectionIndex ? { ...item, title: event.target.value } : item))} placeholder="Section title" /><button type="button" disabled={sections.length <= 1} onClick={() => setSections((items) => items.filter((_, i) => i !== sectionIndex))}><Trash2 size={13} /></button></div>{section.rows.map((row, rowIndex) => <div className="templateItemRow" key={rowIndex}><input value={row.rowId} onChange={(event) => setSections((items) => items.map((item, i) => i === sectionIndex ? { ...item, rows: item.rows.map((current, j) => j === rowIndex ? { ...current, rowId: event.target.value } : current) } : item))} placeholder="row_id" /><input value={row.title} onChange={(event) => setSections((items) => items.map((item, i) => i === sectionIndex ? { ...item, rows: item.rows.map((current, j) => j === rowIndex ? { ...current, title: event.target.value } : current) } : item))} placeholder="Row title" /><button type="button" disabled={section.rows.length <= 1} onClick={() => setSections((items) => items.map((item, i) => i === sectionIndex ? { ...item, rows: item.rows.filter((_, j) => j !== rowIndex) } : item))}><Trash2 size={13} /></button></div>)}{section.rows.length < 10 && <button type="button" className="secondary" onClick={() => setSections((items) => items.map((item, i) => i === sectionIndex ? { ...item, rows: [...item.rows, { rowId: `option_${item.rows.length + 1}`, title: `Option ${item.rows.length + 1}`, description: '' }] } : item))}><Plus size={13} /> Add row</button>}</div>)}{sections.length < 5 && <button type="button" className="secondary" onClick={() => setSections((items) => [...items, { title: `Section ${items.length + 1}`, rows: [{ rowId: `option_${items.length + 1}`, title: 'New option', description: '' }] }])}><Plus size={13} /> Add section</button>}</div>}
