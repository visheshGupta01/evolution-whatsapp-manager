...
export function AudiencesPage(){
 ...
 const [selected,setSelected]=useState<Audience|null>(null); const [editorOpen,setEditorOpen]=useState(false); const [name,setName]=useState(''); const [description,setDescription]=useState(''); const [rows,setRows]=useState<RecipientRow[]>([]); const [fileName,setFileName]=useState(''); const [busy,setBusy]=useState(false); const inputRef=useRef<HTMLInputElement>(null);
 ...
 const close=()=>{setSelected(null);setEditorOpen(false);setName('');setDescription('');setRows([]);setFileName('')};
 const create=()=>{setSelected(null);setEditorOpen(true);setName('');setDescription('');setRows([]);setFileName('')};
 const edit=async(id:string)=>{setBusy(true);try{const a=await audiencesApi.get(id);setSelected(a);setEditorOpen(true);setName(a.name);setDescription(a.description);setRows(a.recipients.map((r,i)=>({...r,row:i+1,valid:true})));setFileName('Saved audience')}catch(e){toast.error(e instanceof Error?e.message:'Could not load audience.')}finally{setBusy(false)}};
 ...
 return <div className='audiencesPage'>
  ...
  {editorOpen&&<div className='audienceBackdrop'><section className='audienceEditor'><header><div><p className='eyebrow'>{selected?'EDIT AUDIENCE':'NEW AUDIENCE'}</p><h2>{selected?'Edit audience':'Create audience'}</h2></div><button className='closeBtn' onClick={close}><X size={18}/></button></header>
